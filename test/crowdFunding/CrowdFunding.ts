...
describe("Unit tests", function () {
  ...
  describe("CrowdFunding", function () {
    ...
    it("should revert you are no the owner", async function () {
      await this.crowdFunding.createFuncding("Test_name", "Test_description", 10000);
      await expect(this.crowdFunding.connect(this.user1).createVoting("Test_title", "Test_description", 0, 20000)).to.be.revertedWith(
        "You are not this CrowdFunding creator.",
      );
    });
    ...
    it("should revert when trying to fund yourself", async function () {
      ...
      await expect(this.crowdFunding.fund(_id, { value: 500 })).to.be.revertedWith("You can't fund yoursel :)");
    });
    it("should revert when trying to fund beyond the proclaimed goal", async function () {
      ...
      await expect(this.crowdFunding.connect(this.user1).fund(_id, { value: goal + 1 })).to.be.revertedWith(
        "Your donation will exceed proclaimed goal",
      );
    });
    it("should revert when funding campaign that achieved its goal", async function () {
      ...
      await expect(this.crowdFunding.connect(this.user2).fund(_id, { value: 1 })).to.be.revertedWith(
        "Your donation will exceed proclaimed goal",
      );
    });
    it("should revert if users donation exceeds current goal", async function () {
      ...
      await this.crowdFunding.createFuncding("Test_name", "Test_description", goal);
      await this.crowdFunding.connect(this.user1).fund(_id, { value: goal - 1 });
      await expect(this.crowdFunding.connect(this.user2).fund(_id, { value: goal })).to.be.revertedWith(
        "Your donation will exceed proclaimed goal",
      );
    });
    it("should revert when funding is not active", async function () {
      ...
      await expect(this.crowdFunding.connect(this.user1).fund(_id, { value: goal })).to.be.revertedWith("The funding target is not active.");
    });
    it("should vote if all requirenments are met", async function () {
      ...
      expect(await this.crowdFunding.connect(this.user1).viewOwnVotes(_voteId)).to.equal(true);
    });
    it("should revert voting is not active", async function () {
      ...
      expect((await this.crowdFunding.fundingVotes(_fundId, _voteId)).isActive).to.equal(false);
    });
    it("should revert voting instance is not active", async function () {
      ...
      await this.crowdFunding.connect(this.user1).fund(_fundId, { value: 3000 });
      await this.crowdFunding.endVote(_fundId, _voteId);
      await expect(this.crowdFunding.connect(this.user1).vote(_fundId, _voteId, true)).to.be.revertedWith("Voting instance is not active.");
    });
    it("should revert already voted", async function () {
      ...
      await this.crowdFunding.createVoting("Test_title", "Test_description", _voteId, 20000);
      await this.crowdFunding.connect(this.user1).vote(_fundId, _voteId, true);
      await expect(this.crowdFunding.connect(this.user1).vote(_fundId, _voteId, true)).to.be.revertedWith("You have already voted.");
    });
    it("shoul return correct summ of donations", async function () {
      ...
      expect(await this.crowdFunding.connect(this.user1).viewOwnContributions(_fundId)).to.equal(_value);
    });
    it("should return correct voting info", async function () {
      ...
      expect(await this.crowdFunding.connect(this.user1).viewOwnVotes(_voteId)).to.equal(_choice);
    });
    it("calculates winner correctly", async function () {
      ...
      expect(await this.crowdFunding.isVotingSuccessful(_fundId, _voteId)).to.equal(true);
    });
    it("calculates winner correctly", async function () {
      const _fundId = 0;
      const _voteId = 0;
      ...
      await this.crowdFunding.connect(this.user3).vote(_fundId, _voteId, false);
      expect(await this.crowdFunding.isVotingSuccessful(_fundId, _voteId)).to.equal(false);
    });
    it("should finish funding", async function () {
      ...
      expect((await this.crowdFunding.crowdFunding(_fundId)).isActive).to.equal(false);
    });
    it("should revert with you are not the owner of this funding campaign", async function () {
      ...
      await expect(this.crowdFunding.connect(this.user1).endFunding(_fundId)).to.be.revertedWith(
        "You are not the owner of this funding campaign",
      );
    });
    it("should revert with funding campaign is already finished", async function () {
      ...
      await expect(this.crowdFunding.endFunding(_fundId)).to.be.revertedWith("This funding campaign is already finished");
    });
    it("should revert with You are not the owner of this funding campaign", async function () {
      const _fundId = 0;
      const _voteId = 0;
      ...
      await this.crowdFunding.createFuncding("Test_name", "Test_description", 100000);
      await this.crowdFunding.createVoting("Test_title", "Test_description", _voteId, 2000);
      await expect(this.crowdFunding.connect(this.user1).endVote(_fundId, _voteId)).to.be.revertedWith(
        "You are not the owner of this funding campaign",
      );
    });
    it("should revert with Voting isn't active", async function () {
      ...
      await this.crowdFunding.endVote(_fundId, _voteId);
      await expect(this.crowdFunding.endVote(_fundId, _voteId)).to.be.revertedWith("Voting isn't active");
    });
  });
});
